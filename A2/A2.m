...
%Part 5
    %fast
        %S1
        [~, S1inlierIdx] = estimateGeometricTransform2D(S1matchedPoints1R,S1matchedPoints2R,'projective', MaxNumTrials=100000, MaxDistance = 30, Confidence = 99.9);
        ...
        figure;
        %S2
        [~, S2inlierIdx] = estimateGeometricTransform2D(S2matchedPoints1R,S2matchedPoints2R,'projective', MaxNumTrials=100000, MaxDistance = 30, Confidence = 99.9);
        ...
        figure;
        %S3 
        [~, S3inlierIdx] = estimateGeometricTransform2D(S3matchedPoints1R,S3matchedPoints2R,'projective', MaxNumTrials=100000, MaxDistance = 30, Confidence = 99.9);
        ...
        figure;
        %S4
        [~, S4inlierIdx] = estimateGeometricTransform2D(S4matchedPoints1R,S4matchedPoints2R,'projective', MaxNumTrials=100000, MaxDistance = 30, Confidence = 99.9);
    ...
    %Panorama production
        for i=2:2:length(tform)*2
            ...
            imageSize = size(panoramaImages{i});
            [xlim, ylim] = outputLimits(tform(i/2), [1 imageSize(2)], [1 imageSize(1)]);
            xmin = min([1; xlim(:)]);
            ymin = min([1; ylim(:)]);
            ...
            panorama = zeros([height width 3], 'like', panoramaImages{i-1});
            ...
            warped = imwarp(panoramaImages{i}, tforms(1), 'OutputView', panoramaView);
            %Create a binary mask
            mask = imwarp(true(size(panoramaImages{i}, 1), size(panoramaImages{i}, 2)), tforms(1), 'OutputView', panoramaView);
            %Overlay warped onto the panorama
            panorama = step(blender, panorama, warped, mask);
            %Transform im1 into the panorama
            warped = imwarp(panoramaImages{i-1}, tforms(2), 'OutputView', panoramaView);
            %Create a binary mask
